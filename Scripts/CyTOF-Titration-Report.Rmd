...
params:
  ...
  ARCSINHCOFAC: 
    ...
    input: numeric 
  DENSITYSQUARES: 
    label: "Amount of squares for Density plot. 100-200 recommended. Higher = more detail & more memory required."
    value: 150
    input: numeric 
  SCORES:
    label: "Names of score types, seperated by commas.*"
    value: "Intensity Score,AOF Score,Concentration Score,Spillover Score (Contributing),Spillover Score (Receiving),Overall / Weighted Score"
    input: text
  K: 
...
plotdensityplot <- function(x, y) {
  ...
  dat <- dat[dat[2] > 0 ,]
  dat$density <- get_density(dat[,1], dat[,2], n = params$DENSITYSQUARES)
  return(ggplot(dat) + geom_point(aes(x, y, color = density), size = 1, show.legend = TRUE) + ylim(0, quantile(exprs[,marker], 0.9995))
 + scale_colour_gradientn(colours = colorRamps::matlab.like2(100)) + labs(title = paste(marker, "-", metalmarkers[[marker]]), x = "Concentration", y = "Expression") + theme(text = element_text(size=20), axis.text.x = element_text(size=20, hjust=1), axis.text.y = element_text(size=20, hjust=1)) + geom_rect(xmin = bestsamples[[marker]]-0.5, ymin = 0, xmax = bestsamples[[marker]]+0.5, ymax = quantile(exprs[,marker], 0.9994), color = "red", fill = NA))
}

## Main Marker Analysis Loop
bestsamples <- list()
for (marker in lineage_channels) {
  cat("<br><br>## ", paste(marker, "-", metalmarkers[[marker]]))
  cat("\n")
  ...
  if (params$SPILLOVER != "Do Nothing") {
    if (marker %in% spillovermarkers) {
      ...
      spilloverreceiving <- data.frame("Percentage Receiving" = compensationmatrix[, marker], row.names = spillovermarkers)
      ...
      tkable <- kable_styling(tkable , bootstrap_options = c("striped", "hover", "responsive"), full_width = FALSE)
      print(tkable)
  ...
  densityplot <- plotdensityplot(x = exprs_spread["Sample"], y = exprs_spread[marker])
  print(densityplot)
  
  ## Save Marker-Specific Plot and Score Table as output files
  ggsave(plot = densityplot, filename = paste("Graph for Marker ", marker, " - ", metalmarkers[[marker]], ".png", sep = ""))
  write.table(t(as.data.frame(scores[[marker]])), sep = "\t", col.names = TRUE, 
              file = paste("Scores for Marker ", marker, " - ", metalmarkers[[marker]], ".tsv", sep = ""))}
