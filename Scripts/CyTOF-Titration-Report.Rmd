...
params:
  ...
  SPILLOVERMATRIX:
    ...
    value: "M:/Shared/CyTOF/Protocols/Titration/Spillover_Fluidigm_Mike.xlsx"
    input: file
  markernames:
  ...
  scores:
    ...
    value: "Intensity,AOF Score,Concentration Score,Spillover Score,Overall / Weighted Score"
    input: text
  k: 
...
if (params$SPILLOVER != "Do Nothing") {
  ...
  if (params$SPILLOVER == "Give Warnings & Compensate Data") {
    ...
    exprs_compensated <- as.data.frame(fcs@exprs)
...
exprs2 <- rbind.fill(exprs_spread)
...
for (marker in lineage_channels) {
  ...
  if (params$SPILLOVER != "Do Nothing") {
    if (marker %in% spillovermarkers) {
      ...
      cat("\n")
      plot.new()
      grid.table(as.data.frame(spilloversummary))
      cat("\n")
    } else {cat("\n- Marker", marker, "was not found in Spillover Table, not checking for spillover / compensating.", sep = " ")}
  }
  ...
  for (sample in 1:samplenumber) {
    ...
    if (!(marker %in% spillovermarkers)) {scores[[marker]][sample,scoretypes[4]] <- 1} else {
      spilloverscore <- 0
      for (spillovermarker in spillovermarkers) {
        if ((spillovermatrixmatrix[spillovermarker,marker] > 0) & (spillovermarker != marker)) {
          ...
          # print(paste(marker, spillovermarker, spilloverpercentage, uncomp_maxspilloverexpression, uncomp_maxspilloverexpression, (uncomp_maxspilloverexpression*(spilloverpercentage/100)), spillovermarkerscore))
  ...
  scores[[marker]][,scoretypes[5]] <- round(rowMeans(scores[[marker]][,1:4]),2) 
  # scores[[marker]][,scoretypes[5]] <- round( rowMeans(scores[[marker]][,1:3]) * scores[[marker]][,4] ,2) 
  bestsamples[[marker]] <- as.numeric(rownames(scores[[marker]])[which.max(scores[[marker]]$`Overall / Weighted Score`)])
  cat("\n")
  plot.new()
  grid.table(as.data.frame(scores[[marker]]))
  cat("\n")
  ...
  plot(exprs2[,c("Sample", marker)], pch=".", col = exprs$FileNum, main=paste(marker, "-", metalmarkers[[marker]]))
  rect(xleft = bestsamples[[marker]]-0.5, ybottom = 0, xright = bestsamples[[marker]]+0.5, ytop = max(exprs[,marker]), border = "red")
  if (params$markernames) {png(paste("Graph for ", marker,".png", sep = ""))} else {png(paste("Graph for ", marker,".png", sep = ""))}
  plot(exprs2[,c("Sample", marker)], pch=".", col = exprs$FileNum, main=paste(marker, "-", metalmarkers[[marker]]))
  rect(xleft = bestsamples[[marker]]-0.5, ybottom = 0, xright = bestsamples[[marker]]+0.5, ytop = max(exprs[,marker]), border = "red")
  dev.off()
  
  ## Write Marker-Specific Score Table
  write.table(t(as.data.frame(scores[[marker]])), sep = "\t", file = paste("Scores for", marker, ".tsv", sep = ""), col.names = TRUE)}
