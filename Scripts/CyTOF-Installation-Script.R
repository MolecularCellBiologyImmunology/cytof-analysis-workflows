...
if (length(installation_needed) == 0) {message("\n- All packages are already installed! No installation needed.\n")} else {
  ...
  if (!(dir.exists(packagefolder))) {message("\nNo directory with Package zip files found. Attempting to install from cloud instead.\n")} else {
    ...
    setwd(packagefolder)
    if (!(unlist(lapply("GenomeInfoDbData_1.2.1.tar.gz", require, character.only = TRUE, quietly = TRUE)))) {install.packages("GenomeInfoDbData_1.2.1.tar.gz", repos = NULL, type = "source", lib = LibPath); require(GenomeInfoDbData)}
